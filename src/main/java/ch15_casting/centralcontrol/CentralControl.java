...
public class CentralControl {
    ...
    public void addDevice(Power device) {
        ...
        deviceArray[emptyIndex] = device;       // private로 적용해놔서 method를 경유했습니다.
        // public 이면 main 에서 그냥 바로 위의 코드를 집어넣으면 되겠죠.
    ...
    private int checkEmpty() {
        // 메서드인데 private를 썼습니다. Main 에서 굳이 몇개나 더 전자제품을 추가할 수 있는지 알 필요가 없을 것 같아서요.
        // deviceArray 라는 배열에서 비어있는 index 를 추출해주기만 하면 되네요.
        // i++니까 가장 빨리 만나게 되는 null 값을 return 하게 될겁니다.
        ...
        return -1;  // 결과값이 없다.
        /*
            Java의 index 넘버에는 음수값이 없기 때문에 (python 에서는 마이너스 인덱스 개념이 있는데 얘도 시험에 자주 나옵니다.),
            실패를 나타낼 때 -1을 쓰는 경우가 많습니다. 하지만 0과 너무 가까운 수이다보니 -100이든지,
            아니면 return 값으로 나올 수 없을만한 음수값을 지정하는 경우도 있는데,
            나중에 위에 있는 addDevice()메서드에서 if(checkEmpty) == -89234)로 쓰고싶지 않으면
            -1 쓰는게 가장 보편적입니다.
         */
    }
    ...
    public void powerOn() {
        ...
        for ( int i = 0; i < deviceArray.length; i++) {
            if(deviceArray[i] == null) {
                ...
                continue;   // break; 는 반복문을 즉시 종료하고, return은 method를 즉시 종료하는데
                            // continue의 경우에는 현재 반복만 종료하고 그 다음 반복문으로 넘어갑니다.
                            // 즉 deviceArray[3] 주소지에 객체가 없어 null이 있다면
                            // 67번 라인의 조건문이 실행될텐데 sout이 출력되고 곧장 deviceArray[4]를확인하게 됩니다.
            }
            deviceArray[i].on();
        }
    }
    ...
    /*
        powerOff() 메서드를 정의하고, Main 에서 호출하시오.
        단, 배열 내부를 탐색하는 반복문을 작성할 때 향상된 for문을 작성하시오.
     */
    ...
    /*
        ...
        deviceArray 를반복 돌려서 몇번 인덱스에 어떤 클래스의 객체가 있는지 표시하는
        showInfo() 메서드를 정의하시오.
        실행 예
        슬롯 [ 1 ] 번 : Computer
        슬롯 [ 2 ] 번 : LED
...
//            int a = 1;
//            if ( arrays == null) {
//                System.out.println("슬롯 [ " + (a++) + " ] 번 : Empty");
//                continue;
//            }
//            System.out.println("슬롯 [ " + (a++) + " ] 번 : " + arrays.getClass().getSimpleName());
//        }
    }
    ...
    public void performSpecificMethod() {
        ...
        for(Power device : deviceArray) {
            if (device instanceof AirConditioner) {     // Power의 서브 클래스가 잘못된 다운캐스팅을 하지 않도록 하는 조건문
            ...
            } else {    // 아직 instanceof 연산자를 통해서 조건문을 추가하지 못한 class 및 추후 업데이트를 통해서
                        // 다른 기기들을 지원할 때는 } else 사이에 else if 문을 추가하면 됨.
    ...
    // 이상의 수업 과정들을 전부 거쳤을 때, Printer 클래스를 정의하고 Power interface를 implement 하고
    // 고유 메서드로 print() -> "프린터가 인쇄를 합니다." 로 출력될 수 있도록 정의하고,
    // Main 에서 printer1 을 centralControl에 추가, on () / off() 를 실행한 후에
