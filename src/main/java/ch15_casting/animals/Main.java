...
//        animal1.fetch();      // 이게 불가능  animal 의 객체이기 때문에 dog 의 고유메서드 호출 불가
...
//        animal2.fetch();        // 불가능
        ...
        /*
            ...
            방법
            (클래스명)객체명; -> (int)2.3;과 같은 방식이죠. conversion의 객체버전이라고 생각하시면 됩니다.
            그래서 기본 자료형의 형변환에서 벗어나서 참조 자료형까지 고려하셔야합니다.
         */

        Dog dog2 = (Dog) animal2;   // 다운 캐스팅 방법 -> 위에 보시면 Animal animal2 = new Dog();로 생성해서 dog2 객체명은 없습니다.
...
//        dog3.makeSound();             // 그러니까 얘는 지금 호출도 못해보는 상황입니다.
        ...
        if(animal4 instanceof Dog) {    // 이 조건식이 true 라면 animal4의 생성시 호출된 생성자는 Dog();
            Dog dog4 = (Dog) animal4;   // 그러므로 95번 라인의 실행문을 통해 다운캐스팅을 하더라도 오류 발생 x
