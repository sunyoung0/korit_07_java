...
/*
    ...
    근데 element 의 값을 조정하거나, 더하거나와 같은 추가적인 가공을 하기 위해서 for 반복문을
    ...
    여기서 중요한 점은 동일한 method 명이라고 하더라도 어떤 클래스에 종속되어있는가에 따라 다른 결과값을 지닐 수 있다는 점입니다.
 */
...
//        System.out.println(num2);
        ...
        // int[] numbers 배열에 1, 2, 3, 4, 5, ... 10 까지 대입해두세요.
        ...
        /*
            ...
            내림차순할 때는 보니까 argument가 두개입니다.
            이상의 과정이 overloading이 이루어졌다는 점을 파악할 수 있으면 현재 수준으로는 만족스럽겠습니다.
            그리고 두번째 argument로 Comparator.reverseOrder();가 사용되었는데,
            Comparator.reverseOrder()의 결과값/return 값이 sort() 메서드의 두번째 argument로 사용되었다는 점에서
            역시 함수형 프로그래밍의 일종이라고 볼 수 있겠네요.
            근데 Comparator.reverseOrder() 메서드를 쓰려면 기본 자료형( 원시 자료형 / primitive type) int[] 배열을
            사용할 수 없어서 Integer[] 배열을 사용하였는데, 현재는 char -> String 처럼 int -> Integer 라고만 생각해두시면 되겠습니다.
            추후 수업 예정
         */
    }
