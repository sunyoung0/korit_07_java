...
/*
    ...
    예를 들어 List를 하나 정의했고, 거기에 element 들을 집어넣었다고 가정하겠습니다.
    그 경우에 List의 element 들을 추출하기 위해서 반복문을 쓰게 될텐데,
    i < strList.size() 를 쓰지 않고 i < 3001 과 같은 방식으로 하드코딩을 했을 때에만 예외가 발생하겠죠.

    그리고 여러분들은 향상된 for 문을 배웠기 때문에, 이를 사용한다면 예외를 볼 일이 전혀 없습니다.
 */
...
//        }
        ...
        int b = 0;
        try {
            int result = a / b;     // ArithmeticException 예외 발생 지점 -> 41번 라인이 아니라 catch 문으로 넘어갑니다.
        ...
        try {
            ...
            throw new RuntimeException(e);      // 사실 저희가 보던 시뻘건게 이 오류 메세지였습니다.
        } finally {
            System.out.println("프로그램 정상 종료");
        }

        /*
            이상의 코드 라인에서 중요한 점은 catch 문이 복수로 나올 수 잇다는 점입니다.
            저희가 if문(leap year 관련 -윤년)에 대해서 작성할 때
