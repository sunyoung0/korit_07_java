...
//        name = scanner.next();      // 입력에 공백을 뒀을 때
//        System.out.println(name);

        /*
            2. next(Int / Double, ...) vs nextLine
                - nextLine은 띄어쓰기를 허용함. enter키, 즉 개행을 기준으로 데이터가 변수에 저장됨
                -> 그래서 28번 실행 결과가 공백 뒤로는 안나왔음.
                - 나머지는 허용 안함. 띄어쓰기 하면 잘려서 대입됨.
         */
...
//        System.out.println("이름 : " + name);
        ...
        /*
            이상의 코드에서 문제점은 39번라인이 실행됐을 때 데이터를 입력 받지 않고 그대로 넘어간다는 점
            왜 이런 문제가 생기나면 nextInt()에서 값을 입력하고 enter를 치게 되는데
            그 경우 다음 nextLine()에서는 enter 키를 받아들여서 빈값인 상태로 enter를 쳤다고 인지하게 되기 때문에
            곧장 41번 출력문으로 넘어갔기 때문
...
//        System.out.println("이름 : " + name);
        ...
        System.out.println("풀이법 2 : nextLine()을 안쓰는 방법");
        System.out.print("점수를 입력하세요 >>> ");
